/**
 * Manages the printer update page interactions and form submissions
 */
class UpdatePrinterManager {
    constructor() {
        // Initialize forms
     ...
     */
    setupEventListeners() {
        // Printer form submission
        this.printerForm.addEventListener('submit', e => this.handlePrinterFormSubmit(e));
    ...
    setupDragAndDrop() {
        ...
        preview.addEventListener('click', async (e) => {
            ...
            if (deleteImage) {
                ...
                if (confirmed) {
                    const printerId = deleteImage.dataset.id;
                    this.sectionLoader.showLoader();
                    try {
                        const response = await fetch(window.AppConfig.urls.printerImageDelete.replace("placeholder", printerId), {
                            method: 'POST',
                            headers: {
                        ...
                        if (data.success) {
                            window.toastManager.buildToast()
                                .setMessage(gettext('Printer image deleted successfully'))
                                .setType('success')
                                .setPosition('top-right')
                                .setDuration(3000)
                                .show();
                            ...
                            window.toastManager.buildToast()
                                .setMessage(gettext(data.error || 'Error deleting printer image'))
                                .setType('danger')
                                .setPosition('top-right')
                                .setDuration(5000)
                                .show();
                        ...
                        window.toastManager.buildToast()
                            .setMessage(gettext('Error deleting printer image: ') + error.message)
                            .setType('danger')
                            .setPosition('top-right')
                            .setDuration(5000)
                            .show();
     ...
     */
    async handlePrinterFormSubmit(event) {
        ...
        try {
            ...
            await this.handleResponse(response, form, this.printerValidators);

            window.toastManager.buildToast()
                .setMessage(gettext('Printer updated successfully'))
                .setType('success')
                .setPosition('top-right')
                .setDuration(3000)
                .show();
            ...
            this.handleError(error, gettext('Error updating printer'), gettext('Please correct the errors and try again.'));
        } finally {
            this.sectionLoader.hideLoader();
        }
    }
...
/**
 * Initialize UpdatePrinterManager on DOM load
 */
document.addEventListener('DOMContentLoaded', () => {
    new UpdatePrinterManager();
});
