/**
 * Manages the leasing printer list page interactions, including details, deletion, sorting, and searching.
 */
class ListPrinterManager {
    constructor() {
        // Initialize section loader
        this.sectionLoader = new SectionLoaderManager(null, 'fill-tertiary', 0.5);

        // Initialize modal
        this.detailsModal = new Modal(document.getElementById('printer-details-modal'), {
            backdrop: 'static',
    ...
    setupEventListeners() {
        ...
        // Search listener
        const searchInput = document.getElementById('printers-search');
    ...
    handleSearch(input) {
        ...
        rows.forEach(row => {
            ...
            if (designation.includes(searchTerm) || serialNumber.includes(searchTerm) || reference.includes(searchTerm) || status.includes(searchTerm)) {
                row.style.display = '';
            } else {
    ...
    async handleDelete(event) {
        ...
        const message = gettext('Are you sure you want to delete the printer "{designation}"?').replace('{designation}', designation);
        ...
        if (confirmed) {
            ...
            try {
                const deleteUrl = window.AppConfig.urls.deletePrinter.replace('placeholder', pk);
                ...
                if (response.ok) {
                    window.toastManager.buildToast()
                        .setMessage(gettext('Leasing printer deleted successfully'))
                        .setType('success')
                    ...
                    window.toastManager.buildToast()
                        .setMessage(gettext('Failed to delete printer: ') + errorData.error)
                        .setType('error')
                        .setPosition('top-right')
                        .setDuration(3000)
                        .show();
    ...
    async handleDetails(event) {
        ...
        try {
            const detailsUrl = window.AppConfig.urls.retrievePrinter.replace('placeholder', pk);
            ...
            if (response.ok) {
                ...
                throw new Error(gettext('Error fetching printer details'));
            }
        } catch (error) {
            console.log(error)
            window.toastManager.buildToast()
                .setMessage(gettext('Error fetching printer details: ') + error.message)
                .setType('error')
                .setPosition('top-right')
                .setDuration(3000)
                .show();
    ...
    /**
     * Populate the details modal with printer data
     * @param {Object} data - Printer data from API
     */
    populateDetailsModal(data) {
        // Update profile picture
        const profilePicture = document.getElementById('details-printer-picture');
        profilePicture.src = data.image_url || "{% static 'images/printer/printer.jpg' %}";
        profilePicture.alt = data.designation ? gettext('Image of ') + data.designation : gettext('Printer Image');

        // Update fields
        document.getElementById('details-designation').textContent = data.designation || "-";
        document.getElementById('details-serial-number').textContent = data.serial_number || "-";
        document.getElementById('details-reference').textContent = data.reference || "-";
        document.getElementById('details-acquisition-date').textContent = data.acquisition_date || "-";
        document.getElementById('details-warranty').textContent = data.warranty || "-";
        document.getElementById('details-end-of-life').textContent = data.end_of_life || "-";
        document.getElementById('details-description').textContent = data.description || "-";
        document.getElementById('details-more-details').textContent = data.more_details || "-";
        ...
        const editButton = document.querySelector('#printer-details-modal .edit-btn');
        editButton.href = window.AppConfig.urls.updatePrinter.replace('placeholder', data.pk);

        const deleteButton = document.querySelector('#printer-details-modal .delete-btn');
...
/**
 * Initialize ListPrinterManager on DOM load
 */
document.addEventListener('DOMContentLoaded', () => {
    new ListPrinterManager();
});
